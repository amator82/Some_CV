...
export class FullPage {
    constructor(element, options) {
        let config = {
            ...
            mode: element.dataset.fpEffect
                ? element.dataset.fpEffect
                : 'slider',
            //===============================
            // Буллеты
            // Активация буллетов
    ...
    checkScroll(yCoord, element) {
        ...
        if (!this.stopEvent && element) {
            ...
            if (this.haveScroll(element)) {
                ...
                if (
                    (Math.abs(position - element.scrollHeight) < 2 &&
                        yCoord <= 0) ||
                    (Math.abs(position - element.clientHeight) < 2 &&
                        yCoord >= 0)
                ) {
                    this.goScroll = true
                }
    ...
    touchMove(e) {
        ...
        if (
            !this.clickOrTouch ||
            e.target.closest(this.options.noEventSelector)
        )
            return
    ...
    choiceOfDirection(direction) {
        ...
        if (
            (this.activeSectionId === 0 && direction < 0) ||
            (this.activeSectionId === this.sections.length - 1 && direction > 0)
        ) {
            this.stopEvent = false
        }
...
if (document.querySelector('[data-fp]')) {
    mtrModules.fullpage = new FullPage(document.querySelector('[data-fp]'), '')
}
